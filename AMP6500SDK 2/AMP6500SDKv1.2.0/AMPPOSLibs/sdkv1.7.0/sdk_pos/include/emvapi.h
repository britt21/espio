...
struct emv_offline_pin {
		int  cipher;
		int fd;
		RsaPinKey_t  pin_key;
		unsigned short apduResp;
};
...
//Call the emv_get_all_candapps or emv_get_candidate_list when callback emv_candidate_apps_selection
//called
...
//emv_build_candidate_list + emv_app_select + emv_app_init == emv_application_select 
//                                                                                                   emv_application_select_ex
int emv_build_candidate_list(int fd, int bPPSE);
int emv_app_select(int ifd,int IgnorBlockError);

//emv_app_init, recall emv_app_select if errno is EMV_ERRNO_SELECT_NEXT
int emv_app_init(int ifd, uint32_t trans_no);
