...
#ifndef QUECTEL_QMI_FEATURE
#define QUECTEL_QMI_FEATURE
#endif
...
#ifdef QUECTEL_QMI_FEATURE
//xtf@2015.12.30 Add start
typedef unsigned long qword[ 2 ];
//xtf@2015.12.30  Add end
#endif
/**
    @}
  */
...
typedef struct {
  ...
  */
#ifdef QUECTEL_QMI_FEATURE  
  uint8_t oem_item_valid; 
  uint32_t oem_item;
  uint8_t oem_item_data_valid;
  uint32_t oem_item_data[31]; 

  uint8_t nv_item_id_valid;
  uint32_t nv_item_id;
  uint8_t mac_address_item_data_valid;
  uint8_t mac_address_item_data[6];

  uint8_t imei_item_data_valid;
  uint8_t imei_item_data[9];
  uint8_t meid_item_data_valid;
  qword meid_item_data;
 
  uint8_t nv_quectel_psn_valid;
  uint8_t nv_quectel_psn[32];
...
typedef struct {
  ...
  */
  ...
  uint8_t voice_oem_outcome_valid;
  uint8_t voice_oem_outcome;

  uint8_t voice_mac_address_outcome_valid;
  uint8_t voice_mac_address_outcome;

  uint8_t voice_imei_outcome_valid;
  uint8_t voice_imei_outcome;

  uint8_t voice_meid_outcome_valid;
  uint8_t voice_meid_outcome;
...
#ifdef QUECTEL_QMI_FEATURE
  uint8_t oem_item_1_valid; 
  uint32_t oem_item_1;
  
  // Added for read NV which response data is within 512 byte
  uint8_t nv_item_id_valid;
  uint32_t nv_item_id;
#endif
...
#ifdef QUECTEL_QMI_FEATURE
  uint8_t oem_item_1_data_valid;
  uint32_t oem_item_1_data[31]; 
  
  // Added for read NV which response data is within 512 byte
  uint8_t nv_item_data_valid; 
  uint8_t nv_item_data[512];  // First byte is the length of data 
#endif
